...
update_Emp = document.querySelector(".update_Emp"),
Emp_Table = document.querySelector(".Emp-Table"),
database = new Localbase("database");
...
/***************(Add Emp)******************/
Add_Emp.addEventListener("submit", (e) => {
  e.preventDefault();
...
/***************(Updata Employee)******************/
update_Emp.addEventListener("submit", (e) => {
  e.preventDefault();
...
//callback function  
Show_Emp_data(Emp_Table);

/************(navbar-tap)************* */
const navTabs = document.querySelectorAll("#btn-head .nav-btn");
const tabsContent = document.querySelectorAll(".resTable .ContentProject");
...
function moveToOtherTab(id) {
  ...
  Add_Emp.classList.add("hide-show");
  update_Emp.classList.remove("hide-show");
  ...
  database
    ...
    .get()
    .then((user) => {
      update_Emp.userID.value = user.id;
      update_Emp.firstName.value = user.firstName;
      update_Emp.lastName.value = user.lastName;
      update_Emp.email.value = user.email;
    });
}
  ...
  database
    ...
    .then((users) => {
      ...
      const data = users.map((user) => {
        return `
          <tr>
            <td class="sub-color">${user.firstName}</td>
            <td>${user.lastName}</td>
            <td class="sub-color">${user.email}</td>
            <td>
      ...
      if (data.length) {  dataList.innerHTML = data.join("");
        ...
        list.parentElement.querySelector( ".ResultTable")
